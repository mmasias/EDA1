        ...
        if (root != null) {
            ...
            while (top >= 0) {
                ...
                if (current != null) {
                    ...
                    result = result + current.getKey();

                    top = top + 1;
                    stack[top] = current.hasRightChild() ? current.getRight() : null;

                    top = top + 1;
                    stack[top] = current.hasLeftChild() ? current.getLeft() : null;
                } else {
                    if (result.length() > 0) {
                        result = result + ",";
                    }
    ...
    public void deserialize(String data) {
        if (data != null && data.length() > 0) {
            ...
            if (index < tokens.length && !tokens[index].equals("null")) {
                root = new Node(Integer.parseInt(tokens[index]));
                index = index + 1;

                Node[] nodeStack = new Node[100];
                int[] stateStack = new int[100];
                int top = -1;
                ...
                while (top >= 0 && index < tokens.length) {
                    ...
                    if (state == 0) {
                        ...
                        if (!tokens[index].equals("null")) {
                            Node leftChild = new Node(Integer.parseInt(tokens[index]));
                            current.attachLeftChild(leftChild);
                            top = top + 1;
                            nodeStack[top] = leftChild;
                            stateStack[top] = 0;
                        }
                        index = index + 1;
                    } else if (state == 1) {
                        stateStack[top] = 2;
                        if (!tokens[index].equals("null")) {
                            Node rightChild = new Node(Integer.parseInt(tokens[index]));
                            current.attachRightChild(rightChild);
                            top = top + 1;
                            nodeStack[top] = rightChild;
                            stateStack[top] = 0;
                        }
                        index = index + 1;
                    } else {
